...
export default function createWindow(windowName,options) {
  ...
  const ensureVisibleOnSomeDisplay = windowState => (!screen.getAllDisplays().some(display=>windowWithinBounds(windowState,display.bounds)))?resetToDefaults():windowState /* <==== Window is partially or fully not visible now. Reset it to safe defaults. */
  const saveState = () => {
